...
export async function listFilledQrsController(req, res) {
  try {
    ...
    const data = await listQrsService({
      type,
      status: "FILLED",
      page,
      limit,
...
export async function updateQrImagesController(req, res) {
  try {
    ...
    if (files.length > 3) {
      return res
        .status(400)
        .json({ success: false, message: "Maximum 3 images allowed" });
    }
    ...
    if (rv_vessel_id !== undefined)
      updatesFromBody.rv_vessel_id = rv_vessel_id === "" ? null : rv_vessel_id;
    if (fish_id !== undefined)
      updatesFromBody.fish_id = fish_id === "" ? null : fish_id;
    if (owner_id !== undefined)
      updatesFromBody.owner_id = owner_id === "" ? null : owner_id;
    if (date !== undefined) updatesFromBody.date = date === "" ? null : date;
    if (time !== undefined) updatesFromBody.time = time === "" ? null : time;
    if (trip_id !== undefined)
      updatesFromBody.trip_id = trip_id === "" ? null : trip_id;
    if (status !== undefined) updatesFromBody.status = status;
    if (latitude !== undefined)
      updatesFromBody.latitude = latitude === "" ? null : parseFloat(latitude);
    if (longitude !== undefined)
      updatesFromBody.longitude =
        longitude === "" ? null : parseFloat(longitude);

    // Update QR record with image information and optional fill fields
    const updatedQr = await updateQrWithImages(
      code,
      uploadedImages,
      updatesFromBody,
    );
    ...
    if (updatedQr.owner_id) {
      await updateOwner(updatedQr.owner_id, {
        owner_register_progress: "COMPLETED",
      });
    }
...
export async function fillQrWithQcDetailsController(req, res) {
  try {
    const { code } = req.params;
    const {
      fish_images: fishImages,
      pond_condition_image: pondConditionImage,
    } = req.files || {};
    ...
    if (!qr.owner_id) {
      return res.status(400).json({
        success: false,
        message: "QR must have an owner before QC filling",
      });
    }

    const owner = await db("rootverse_users")
      .where({ id: qr.owner_id })
      .first();
    if (!owner) {
      return res
        .status(400)
    ...
    if (owner.owner_register_progress !== "COMPLETED") {
      return res.status(400).json({
        success: false,
        message: "Owner registration must be completed before QC filling",
      });
    }
    ...
    if (!quality_checker_id) {
      return res
        .status(400)
        .json({ success: false, message: "quality_checker_id is required" });
    }

    // Convert numeric string fields from multipart form data
    const parsedQcScore =
      qc_score !== undefined && qc_score !== ""
        ? parseInt(qc_score)
        : undefined;
    ...
    const parsedWaterTemperature =
      water_temperature !== undefined && water_temperature !== ""
        ? parseFloat(water_temperature)
        : undefined;
    const parsedPhLevel =
      ph_level !== undefined && ph_level !== ""
        ? parseFloat(ph_level)
        : undefined;
    const parsedGrade =
      grade !== undefined && grade !== "" ? parseInt(grade) : undefined;
    const parsedOdorScore =
      odor_score !== undefined && odor_score !== ""
        ? parseInt(odor_score)
        : undefined;
    const parsedFirmnessScore =
      firmness_score !== undefined && firmness_score !== ""
        ? parseInt(firmness_score)
        : undefined;
    const parsedWeight =
    ...
    if (qc_result && !validQcResults.includes(qc_result)) {
      return res.status(400).json({
        success: false,
        message: "Invalid qc_result. Must be PASS, HOLD, or REJECT",
      });
    }
    ...
    if (quality_grade && !validQualityGrades.includes(quality_grade)) {
      return res.status(400).json({
        success: false,
        message: "Invalid quality_grade. Must be A, B, C, or REJECTED",
      });
    }
    ...
    for (const score of organolepticScores) {
      if (score !== undefined && (score < 0 || score > 5)) {
        return res.status(400).json({
          success: false,
          message: "Organoleptic scores must be between 0 and 5",
        });
      }
    }
    ...
    if (damage && !validDamages.includes(damage)) {
      return res.status(400).json({
        success: false,
        message: "Invalid damage. Must be NONE, MINOR, MODERATE, or SEVERE",
      });
    }
    ...
    if (parsedGrade !== undefined && !validGrades.includes(parsedGrade)) {
      return res
        .status(400)
        .json({ success: false, message: "Invalid grade. Must be 30 or 40" });
    }
    ...
    if (parsedQcScore !== undefined && !validQcScores.includes(parsedQcScore)) {
      return res.status(400).json({
        success: false,
        message: "Invalid qc_score. Must be 0, 10, 20, 30, or 40",
      });
    }
    ...
    if (fishImages && fishImages.length > 0) {
      if (fishImages.length > 5) {
        return res
          .status(400)
          .json({ success: false, message: "Maximum 5 fish images allowed" });
      }

      // Upload fish images to Supabase
      uploadedFishImages = await uploadImagesToSupabase(
        fishImages,
        `${code}_fish`,
      );
    }
    ...
    // Add optional fields
    if (rv_vessel_id !== undefined)
      updates.rv_vessel_id = rv_vessel_id === "" ? null : rv_vessel_id;
    if (fish_id !== undefined)
      updates.fish_id = fish_id === "" ? null : fish_id;
    if (owner_id !== undefined)
      updates.owner_id = owner_id === "" ? null : owner_id;
    if (trip_id !== undefined)
      updates.trip_id = trip_id === "" ? null : trip_id;
    ...
    if (latitude !== undefined)
      updates.latitude = latitude === "" ? null : parseFloat(latitude);
    if (longitude !== undefined)
      updates.longitude = longitude === "" ? null : parseFloat(longitude);
    ...
    if (parsedTemperature !== undefined)
      updates.temperature_c = parsedTemperature;
    if (size) updates.size = size;
    if (damage) updates.damage = damage;
    if (parsedWaterTemperature !== undefined)
      updates.water_temperature = parsedWaterTemperature;
    ...
    if (parsedFirmnessScore !== undefined)
      updates.firmness_score = parsedFirmnessScore;
  ...
  try {
    ...
    if (!qr) {
      return res.status(404).json({
        success: false,
        message: "QR not found with the specified status and code",
      });
    }
    res.status(200).json({ success: true, qr });
  } catch (err) {
...
export const getAllCatchlogsController = async (req, res) => {
  try {
    const query = req.query;
    const data = await getAllCatchlogsService(query);
